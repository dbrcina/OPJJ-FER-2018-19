 ...
 * ArrayIndexCollection which implements {@link List} interface. General
 * contract of this list is: duplicate elements are allowed; storage of
 * {@code non-null} references is not allowed.
 * 
 * @author dbrcina
 * @version 1.0
...
public class ArrayIndexedCollection<T> implements List<T> {
	...
	/**
	 * Adds the given T {@code value} into this collection (reference is added into
	 * first empty place in the internal array; if the internal array is full, it
	 * should be reallocated by doubling its size). The method should refuse to add
	 * {@code null} as element by throwing an appropriate exception
	...
	private static class ArrayListElementsGetter<E> implements ElementsGetter<E> {
		 ...
		 *                          method.
		 */
		public ArrayListElementsGetter(ArrayIndexedCollection<E> col, long modificationCount) {
			this.col = col;
