...
//		}
	...
	private void parseTag() {
		...
		if (currentType == SmartScriptTokenType.VARIABLE && tagName.equals("FOR")) {
			parseFor();
		} else if (currentType == SmartScriptTokenType.VARIABLE && tagName.equals("NOW")) {
			parseNow();
	...
	private void parseNow() {
		...
		if (nextToken.getType() == SmartScriptTokenType.END_TAG) {
			((Node) stack.peek()).addChildNode(new NowNode(null));
			lexer.setState(SmartScriptLexerState.TEXT);
		} else {
			((Node) stack.peek()).addChildNode(new NowNode((String)nextToken.getValue()));
		}
	}
	/**
