 ...
 * LinkedListIndexedCollection which implements {@link List} interface. General
 * contract of this collection is: duplicate elements are allowed (each of those
 * element will be held in different list node); storage of {@code non-null}
...
public class LinkedListIndexedCollection<T> implements List<T> {
	...
	public LinkedListIndexedCollection(Collection<? extends T> other) {
		Objects.requireNonNull(other, "Kolekcija je null!");
		addAll(other);
	}
	 ...
	 */
	private void atFirst(ListNode<T> newNode) {
		newNode.next = first;
	 ...
	 * @param newNode a new node that needs to be inserted.
	 */
	private void atLast(ListNode<T> newNode) {
		last.next = newNode;
		newNode.previous = last;
		last = newNode;
	}
	 ...
	 *                 inserted.
	 */
	private void fromBegining(ListNode<T> newNode, int position) {
	 ...
	 *                 inserted.
	 */
	private void fromEnd(ListNode<T> newNode, int position) {
	 ...
	 */
	private void changeReferences(ListNode<T> newNode, ListNode<T> temp) {
		newNode.next = temp;
	 ...
	 * one for next node and one for previous node, also has one reference for an E
	 * value.
	 * 
	 * @author dbrcina
	 * @version 1.0
	...
	private static class LinkedListElementsGetter<E> implements ElementsGetter<E> {
		 ...
		 * 
		 * @param col               an {@link LinkedListIndexedCollection} type
		 *                          collection.
		 ...
		 *                          method.
		 */
		public LinkedListElementsGetter(LinkedListIndexedCollection<E> col, long modificationsCount) {
			this.col = col;
			cursor = col.first;
