...
public class Ekstenzije {
	...
	public static void main(String[] args) {
		...
		var rezultat = brojiEkstenzije(staza);
		...
		List<Map.Entry<String, Integer>> lista = new ArrayList<>(rezultat.entrySet());
	...
	public static Map<String, Integer> brojiEkstenzije(File staza) {
		if (!staza.isDirectory()) {
			throw new RuntimeException("Staza mora biti direktorij!");
		}
		Map<String, Integer> mapa = new HashMap<>();
		brojiEkstenzijeRekurzivno(staza, mapa);
		return mapa;
	}

	private static void brojiEkstenzijeRekurzivno(File dir, Map<String, Integer> mapa) {
		...
		for (File f : djeca) {
			if (f.isDirectory()) {
				brojiEkstenzijeRekurzivno(f, mapa);
			} else if (f.isFile()) {
